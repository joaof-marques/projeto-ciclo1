from database.database import User, LogUser, engine
...
def create_user(name, email, cpf, password, access_level, logged_user_id):
    ...
    with Session(bind=engine) as session:        
        try:
            ...
            new_user = User(name = name, email = email, cpf = cpf, password=str(hashed_password.decode('utf-8')), access_level = access_level, deleted=False)
